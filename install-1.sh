#!/bin/bash
# URL file allowed_ips.txt di GitHub
# URL file allowed_ips.txt di GitHub
allowed_ips_url="https://raw.githubusercontent.com/MamzStore/mamztes/main/allowed_ips.txt"

# Download file allowed_ips.txt
curl -s -o /tmp/allowed_ips.txt $allowed_ips_url

# Membaca daftar IP yang diizinkan ke dalam array
mapfile -t allowed_ips < /tmp/allowed_ips.txt

# Mendapatkan IP VPS saat ini
current_ip=$(curl -s ifconfig.me)

# Fungsi untuk memeriksa apakah IP saat ini diizinkan
ip_allowed() {
...
# Memeriksa IP dan menjalankan script hanya jika IP diizinkan
if ip_allowed; then
    echo "IP $current_ip diizinkan. Melanjutkan instalasi tunneling..."
#
#  |=================================================================================|
...
cd /etc/nginx
wget -O /etc/nginx/nginx.conf "https://www.rerechan02.com/nginx.conf"
cd
clear
...
echo -e "Installasi Telah Selesai"
    ...
    echo "IP $current_ip tidak diizinkan. Membatalkan instalasi."
    exit 1
fi